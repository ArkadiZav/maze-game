...
void NetworkHandler::InitListener(SOCKET *_socket, unsigned short _port)
{
	...
	int optval = 1;
	if ( 0 > setsockopt(*_socket, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)))
	{
...
void NetworkHandler::GetInput(size_t *_socketid, std::string *_input)
{
	while (true)
	{
		if (!m_totalSocketsReady)
		{
			...
			for (size_t i = 0 ; i < m_numListeners ; ++i)
			{
			...
			if (m_totalSocketsReady == static_cast<size_t>(SOCKET_ERROR))
			{
